...
async function ensureGuestSeatsTable() {
  ...
  await pool.query('ALTER TABLE events ADD COLUMN IF NOT EXISTS logo_data BYTEA');
  await pool.query('ALTER TABLE events ADD COLUMN IF NOT EXISTS logo_mime TEXT');
  await pool.query('ALTER TABLE events ADD COLUMN IF NOT EXISTS brand_color TEXT');
  await pool.query('ALTER TABLE events ADD COLUMN IF NOT EXISTS venue TEXT');
  await pool.query('ALTER TABLE events ADD COLUMN IF NOT EXISTS event_code TEXT');
  ...
  await pool.query('ALTER TABLE guests ADD COLUMN IF NOT EXISTS first_name TEXT');
  await pool.query('ALTER TABLE guests ADD COLUMN IF NOT EXISTS last_name TEXT');
